...
function plotmulti(x,y, x3d, y3d, z3d; c = (:black, 0.1))
    ...
    ax3 = Axis(g2d[2,1:2], xlabel = "x", ylabel = "y", backgroundcolor = :black)
    axS1 = Axis3(g3d[1,1], aspect = :data, perspectiveness = 0.5,elevation = π / 9)
    ...
    wireframe!(axS1, x3d, y3d, z3d; color = c, transparency = true,
        #overdraw = true,
        linewidth = 1)
    meshscatter!(axS1,Point3f(0.8,0,0); color = :white, markersize = 0.35,
    ...
    meshscatter!(axS2, Point3f(0.8,0,0); color = :white, markersize = 0.35,
        transparency = true, backlight = 2f0)
    ...
    text!(s, L"f(x,y) = -2\,x\, e^{-x^2 - y^2}", position=(100, 700/2), space=:pixel)
    text!(ax3, L"f(x) = x\,\sin(3x)/(\cos(x) + 2)", position = (7, 0.8), color = :grey70)
    rowgap!(g2d, 2)
    colgap!(g2d, 2)
    colgap!(g2d, 2)
    rowgap!(fig.layout, 2)
